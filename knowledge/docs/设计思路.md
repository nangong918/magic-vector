...
AI目前是不可能实现实时AI模型的，暂时先这样开发出一版本吧。因为现在的AI很缺少实时多模态能力，就是不断的调用接口。存在世界帧率。然后实时获取反馈，
以后的AI肯定连做接口管理都不需要，全部都是实时的。哎，我也不知道现在做这些有什么意义。就像当初用BERT模型 + TextCNN + 实体命名识别实现槽填充一样，过了一年就出现了AI大模型。
先做吧，也算是对自己能力的暂时提升，通过他学习计算机架构，后续就算AI升级了之后，用已有框架升级也很快。也有一套成熟的AI方案。起码嵌入式设备变得没那么快。
...
##### 模式
* 模式1 简单模式: audio -> stt -> llm -> tts -> audioResult
* 模式2 视觉理解: audio -> stt -> llm-tool -> functionCall -> visionModel -> visonResult -> llm-functionCall -> tts -> audioResult
* 模式3 运动模式: audio -> stt -> llm-mcp -> motionResult -> tts(tts + motion) -> audioResult(audio + motion)
* 模式4 情感模式: audio -> stt -> llm-mcp -> emoResult -> tts(tts + 前端emoji表情) -> audioResult(audio + 前端emoji表情)
* 模式5 database模式: audio -> stt -> llm-mcp -> databaseResult -> tts -> audioResult
* 模式6 联网mcp模式: audio -> stt -> llm-mcp -> tts -> audioResult
* 打断: 前端的小模型, 对AI进行实时打断(前端小模型可靠性差)

* 混合1: 视觉 + 运动 + 情感情趣: audio -> stt -> llm-tool-mcp -> tts(tts + event) -> audioResult(audio + event)
* 混合2: 自调用模式: event(userMotion\time) -> llm-mcp\tool -> result(audio\motion\前端emoji表情)
...
* 图片理解
  * Spring: 
    * Agent调用Function Call告知Android进行上传图片(暂时使用Http上传)
    * Http在接收到图片和会话消息之后将Message通过SpringEvent的方式调用VisionChat
    * VisionChat的Result交给工作流中的下一个Agent处理
  * Android
    * 从YOLO的Camera中抽取一帧交给Spring后端
...
* realtime 聊天
  ...
  * Spring
    * ChatController
    * RealtimeChat WS
      * 开始聊天: 创建messageId, timestamp
        * 创建当前会话缓存(messageId, currentContent, createTimestamp)
      * 传输消息: agentId, content, messageId, timestamp
        * 将会话缓存添加到相应中
      * 传输结束: agentId, messageId, timestamp
        * 清空会话缓存

### 视觉
* Spring：
  * Android传递让AI查看当前场景的chat请求
  * SpringAI理解意图，并调用VLService的MCP去调用ws传递拍摄请求。
  * Spring存储chat请求在ChatContextManager中
  * 图片Base64接收：
    * Spring传递让Android拍摄当前图片的标识符：take_photo；标记分配的messageId
    * Android通过ws返回图片的base64 json，包括timestamp，agentId，messageId，userId；并标记为photo
    * ws接收到photo并将base64数据流交给VLService
    * VLService获取结果并通过ws将结果传递给Android
* Android：
  * Android传递user的question
  * Android接收到Spring的识别结果并截取一帧当前的视频帧
  * 检查当前视频帧是否被矩阵变换，如果变换则改编为正确的方向
  * 对截图视频流进行压缩
  * 通过ws传递给Spring
  * 接收Spring的message、行为MCP（调用表情，移动【看看是什么？-> 移动mcp / 心情表现mcp】）

### 嵌入式模块

头部用ESP32-S3, 核心用RK3566\RK3588
...
### 模型
* LLM：deepseek-v3.1 / qwen-plus 
  * deepseek: 不包含结构化输出；qwen-plus: 包含结构化输出 
  * 最大输入长度：96K（token：98,304）
* VL：qwen3-vl-flash
  * 最大输入长度：256K
  * 单个图像的大小不超过10 MB
  * Base64编码的图像，需保证编码后的字符串小于10MB
  * 图像的宽度和高度均需大于 10 像素，图像的宽高比（长边与短边的比值）不得超过 200
  * 视频流：2s~40s
* STT：fun-asr-realtime
  * 支持的采样率：16kHz
  * 调整格式：FFmpeg
  * STT需要包含重连机制
* TTS：Qwen-TTS，voice-enrollment（声音复刻模型）
  * 最大输入token：512（大约512汉字长度）-> 分片策略: 单句快超过450汉字的时候拆分句子。