...
取消阻塞LLM，改为流式LLM。流式JSON输出：流式输出数据，然后动态判断是否为JSON。如果是JSON就进行发送。
阻塞模式 + 提前上传模式
...
增加可以选择的模式，在对话的时候Android就开始录制或者拍照，然后交给后端，具体是否使用是由后端决定。
解耦websocket和Mqtt。封装抽象成接口

Base64 <-> byte 编解码异步：可能考虑需要使用异步, 如果录音数据并不是很影响写入就不异步了

code review: SpringBoot, Android的代码扫描工具

UDP 传输数据出错, 数据被截断 -> 改为WebRTC
...
TTS超出最大输入限制
```text
com.alibaba.dashscope.exception.ApiException: {"statusCode":400,"message":"<400> InternalError.Algo.InvalidParameter: Range of input length should be [0, 600]","code":"InvalidParameter","isJson":true,"requestId":"5c39aad6-01ec-4471-9880-c5c85436ee34"}
	at com.alibaba.dashscope.protocol.okhttp.OkHttpHttpClient.handleSSEEvent(OkHttpHttpClient.java:303) ~[dashscope-sdk-java-2.21.11.jar:na]
	at com.alibaba.dashscope.protocol.okhttp.OkHttpHttpClient.access$300(OkHttpHttpClient.java:44) ~[dashscope-sdk-java-2.21.11.jar:na]
	at com.alibaba.dashscope.protocol.okhttp.OkHttpHttpClient$2.onEvent(OkHttpHttpClient.java:368) ~[dashscope-sdk-java-2.21.11.jar:na]
