...
| 点击选择Agent闪退 | 点击选择Agent进入chat页面时，应用直接闪退，错误信息为“Cannot create an instance of class com.magicvector.viewModel.activity.ComposeChatVm”。 | 点击Agent后应正常进入聊天页面，无闪退。 | 修复 ComposeChatVm 的构造函数，使其与 ComposeAgentChatVm 保持一致的无参构造函数实现。 | ComposeChatVm 缺少无参构造函数，导致 ViewModelProvider 无法通过默认的 ViewModelProvider.AndroidViewModelFactory 创建实例。 |
| 主页面下拉刷新持续触发并产生大量请求 | MessageList 首页未手动下拉时也持续进入刷新态，并重复发起 `getAgentList/getLastAgentChatList` 请求。 | 下拉刷新只应在用户手势、网络恢复、Agent增删改后触发一次，不应自激循环。 | 移除 `MainActivityScreen -> MineVm.homeRefreshToken -> MessageListScreen.refreshToken` 这条中转刷新链；不再通过 `MineVm` 传递首页刷新 token，而是让 `MessageListMviVm` 直接订阅全局 `NetworkManager.state` 处理网络恢复刷新；Agent 创建/更新/删除成功后由 `MainVm` 直接触发 `messageListVm.Refresh`。 | 根因是首页刷新通过两个 `LaunchedEffect`（`agentListVersion`、`networkState`）不断写入 `MineVm.homeRefreshToken`，而 `MessageListScreen` 又在 `refreshToken` 变化时自动调用 `Refresh`；刷新后再次同步 `AgentsManager`，进而再次推进 `agentListVersion`，形成状态驱动的自循环请求链。 |
