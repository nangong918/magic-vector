...
class AgentsManager {
    private val _agentList = MutableStateFlow<List<AgentModel>>(emptyList())
    val agentList: StateFlow<List<AgentModel>> = _agentList.asStateFlow()
    ...
    fun setAgents(list: List<AgentModel>) {
        _agentList.value = list
        _effect.tryEmit(AgentsEffect.AgentListChanged)
    }

    fun upsertAgent(agent: AgentModel?) {
