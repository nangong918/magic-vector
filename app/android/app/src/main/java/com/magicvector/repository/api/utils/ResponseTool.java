...
import com.magicvector.repository.api.handler.TriConsumer;
...
public abstract class ResponseTool extends ResponseUtil {
    ...
    public static <T> boolean handleResponse(BaseResponse<T> response, Context context){
        if(response != null && response.getCode() != null){
            ...
            if(context instanceof Activity){
                ((Activity) context).runOnUiThread(() -> Toast.makeText(context, context.getString(com.view.appview.R.string.please_check_your_network), Toast.LENGTH_LONG).show());
            }
            return false;
        }
    }
